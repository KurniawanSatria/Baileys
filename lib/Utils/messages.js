    ...
    if ('sections' in message && !!message.sections) {
        ...
        const slides = await Promise.all(message.cards.map(async (slide) => {
            ...
            if (product) {
                ...
                const { imageMessage } = await prepareWAMessageMedia({ image: normalizedProductImage, ...options }, options)
                header = {
                    productMessage: {
                        product: {
                            ...product,
                ...
                header = await prepareWAMessageMedia({ image: normalizedImage, ...options }, options)
            }

            else if (video) {
                const normalizedVideo = normalizeMediaInput(video)
                header = await prepareWAMessageMedia({ video: normalizedVideo, ...options }, options)
            }
